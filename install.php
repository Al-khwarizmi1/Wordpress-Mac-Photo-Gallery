...
/*
  ...
  Description: Wordpress Mac Doc Photogallery Installation file
  Version: 1.0
  Author: Apptha
...
function create_mac_folder() {
    $org_path = dirname(dirname(dirname(__FILE__))) . '/uploads/mac-dock-gallery';
    // to mkdir() must be specified.
    if (is_dir($org_path)){} else {
        mkdir($org_path, 0777, TRUE);
    }
    $src = dirname(dirname(dirname(__FILE__))) . '/plugins/mac-dock-gallery/mac-dock-gallery';
    rename($src, $org_path);
}
