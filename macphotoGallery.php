...
/*
  ...
  Description: Wordpress Mac Doc Photogallery Photo Gallery file
  Version: 1.0
  Author: Apptha
    ...
    if(document.getElementById('mac-test-list'))
                {
                 ...
                 update : function () {
                    ...
                    var order =dragdr('#mac-test-list').sortable('serialize');
                     dragdr.post(site_url+"/wp-admin/admin-ajax.php?action=process-sortable&"+order+"&pagestart="+pagestart);
                   }
           ...
           dragdr('#swfupload-control').swfupload({
                upload_url: site_url+"/wp-admin/admin-ajax.php?action=macphotoupload&albumId=<?php echo $_REQUEST['albid'] ?>",
